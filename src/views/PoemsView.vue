...
import { adminApi, type AdminPoem } from '@/api/admin'
...
async function load() {
  ...
  error.value   = ''
  try {
    const res   = await adminApi.listPoems(page.value, limit)
            ...
            <tr v-for="p in poems" :key="p.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3">
                <span
                  ...
                  :class="p.status === 'published' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'"
                >
                  {{ p.status }}
                </span>
              </td>

...
import { adminApi, type AdminLike } from '@/api/admin'
import Pagination from '@/components/Pagination.vue'

const items   = ref<AdminLike[]>([])
const total   = ref(0)
const page    = ref(1)
const limit   = 20
...
async function load() {
  ...
  error.value   = ''
  try {
    const res   = await adminApi.listLikes(page.value, limit)
...
<template>
  <div class="p-8">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">Likes</h2>
